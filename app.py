...
st.markdown(
    ...
    /* Todos los botones (destacados "primary" y normales "secondary",
       que en modo oscuro se ven negros) en naranja */
    ...
    /* Botones en azul claro con letra negra (Nuevo centro, Datos del
       centro, Nuevo detector, Generar documentos): se marcan con
       type="tertiary" en Python, así el CSS engancha directamente al
       atributo real del botón en vez de depender de su posición en la
       página (más fiable). "Guardar centro" NO lleva este tipo, se
       queda con el color normal. */
    ...
    /* Botón "Eliminar" (marcado con un div justo antes, ver código
       Python): diminuto, ajustado al ancho del texto/icono, en un
       rosa tenue para que quede claramente secundario frente a "Abrir". */
    div:has(> div.marcador-btn-eliminar) + div button {
        background-color: #d98ca0 !important;
    ...
    div:has(> div.marcador-btn-eliminar) + div button:hover {
        background-color: #c1708a !important;
        border-color: #c1708a !important;
        opacity: 1;
    }
...
def init_db():
    ...
    # Planos del centro: hasta 3 por centro, cada uno con su nombre
...
def get_centro(centro_id):
    conn = sqlite3.connect(get_db_path())
    c = conn.cursor()
    c.execute("SELECT id, nombre, zona, fecha_medicion, imagen_exterior_path, tecnico FROM centros WHERE id=?", (centro_id,))
    row = c.fetchone()
    conn.close()
    return row
...
def generar_pdf(centro_id, output_path):
    try:
        ...
        _, nombre, zona, fecha, img_ext, tecnico = centro
        ...
        for idx, d in enumerate(detectores, 1):
            ...
            filas_texto = [
                ...
                ["Fecha optima retirada", _escapar_pdf(fecha_retirada_optima_pdf) or "-", "Fecha real de retirada", ""],
            ]
            filas_pdf = []
    ...
    if "Planos" in wb.sheetnames:
        ...
        for img in ws_p._images:
            try:
                ...
                continue
        orden = 0
        for fila_p in range(2, ws_p.max_row + 1):
...
def generar_excel(centro_id, output_path):
    ...
    cid, nombre, zona, fecha, img_centro, tecnico = centro
    detectores = fetch_detectores(centro_id)

    FUENTE = "Arial"
    fuente_normal = Font(name=FUENTE, size=8)
    ...
    for col in range(16, len(headers) + 1):
    ...
    ws_planos.cell(row=1, column=1, value="Nombre").font = fuente_cabecera
    ws_planos.cell(row=1, column=1).fill = PatternFill("solid", fgColor="F5A623")
    ws_planos.cell(row=1, column=2, value="Imagen").font = fuente_cabecera
    ws_planos.cell(row=1, column=2).fill = PatternFill("solid", fgColor="F5A623")
...
def boton_compartir_whatsapp_archivo(ruta, nombre_archivo, mime_type, texto_mensaje,
    ...
    <div style="font-family: 'Source Sans Pro', sans-serif;">
      <button id="btn-compartir-{id_sufijo}" style="
          background-color:#25D366; color:white; border:none;
          padding:12px 20px; border-radius:8px; font-size:16px;
          font-weight:600; cursor:pointer; width:100%;">
        {etiqueta_boton}
      </button>
      <p id="msg-compartir-{id_sufijo}" style="margin-top:8px; font-size:13px; color:#ccc;"></p>
    </div>
    <script>
      ...
      const btn_{id_sufijo} = document.getElementById("btn-compartir-{id_sufijo}");
      const msg_{id_sufijo} = document.getElementById("msg-compartir-{id_sufijo}");
      ...
      btn_{id_sufijo}.addEventListener("click", async () => {{
        try {{
          ...
          // Se intenta compartir directamente, SIN pasar antes por
          // navigator.canShare(): en algunos dispositivos esa
          // comprobación previa da "false" de forma demasiado
          // conservadora aunque el envío real sí funcionaría.
          ...
          }} else if (err.name === "TypeError" || err.name === "NotSupportedError") {{
            try {{
              await navigator.share({{ title: tituloCompartir_{id_sufijo}, text: textoMensaje_{id_sufijo} }});
              msg_{id_sufijo}.textContent = "Tu navegador no admite adjuntar el archivo directamente; descárgalo y compártelo manualmente.";
            }} catch (err2) {{
              msg_{id_sufijo}.textContent = "Tu navegador no soporta compartir archivos. Descárgalo con el botón de abajo y compártelo manualmente desde WhatsApp.";
            }}
          }} else if (err.name === "NotAllowedError") {{
            msg_{id_sufijo}.textContent = "Tu navegador/dispositivo ha bloqueado el envío directo de este archivo. Descárgalo con el botón de abajo y adjúntalo manualmente en WhatsApp.";
          }} else {{
...
def boton_compartir_whatsapp(pdf_path, nombre_archivo, texto_mensaje):
    ...
    boton_compartir_whatsapp_archivo(
        ...
        "📲 Enviar por WhatsApp", "pdf", "Informe de detectores de Rn",
    )
...
def boton_compartir_whatsapp_fotos(imagenes, texto_mensaje):
    ...
    checklist (sin el PDF).

    Primero intenta enviarlas TODAS JUNTAS en un solo toque (como hacía
    la app antes): con imágenes esto funciona bien en la gran mayoría
    de navegadores/Android, a diferencia de otros tipos de archivo más
    restringidos. Si ese envío conjunto falla (algún dispositivo más
    limitado, demasiadas fotos, etc.), el propio botón cae
    automáticamente al modo "una a una", pidiendo un toque por foto,
    para que el envío nunca se quede completamente bloqueado.
    ...
    html = f"""
    <div style="font-family: 'Source Sans Pro', sans-serif;">
      <p id="progreso-fotos" style="margin:0 0 6px 0; font-size:13px; color:#ccc;"></p>
      <button id="btn-compartir-fotos" style="
          background-color:#25D366; color:white; border:none;
          padding:12px 20px; border-radius:8px; font-size:16px;
          font-weight:600; cursor:pointer; width:100%;">
        📲 Enviar {n_fotos} foto(s) por WhatsApp
      </button>
      <p id="msg-compartir-fotos" style="margin-top:8px; font-size:13px; color:#ccc;"></p>
    </div>
    <script>
      const archivosDataFotos = {archivos_json};
      const textoMensajeFotos = {texto_json};
      let modoUnaAUna = false;
      let indiceFoto = 0;
      ...
      function construirFile(a) {{
        return new File([b64ToBlobFotos(a.b64, a.type)], a.name, {{ type: a.type }});
      }}

      const btnFotos = document.getElementById("btn-compartir-fotos");
      const msgFotos = document.getElementById("msg-compartir-fotos");
      const progresoFotos = document.getElementById("progreso-fotos");

      function actualizarUIPaso() {{
        if (indiceFoto >= archivosDataFotos.length) {{
          progresoFotos.textContent = `✅ Las ${{archivosDataFotos.length}} foto(s) se han enviado.`;
          btnFotos.style.display = "none";
          return;
        }}
        const total = archivosDataFotos.length;
        progresoFotos.textContent = `Modo una a una — Foto ${{indiceFoto + 1}} de ${{total}}: ${{archivosDataFotos[indiceFoto].name}}`;
        btnFotos.textContent = `📲 Enviar esta foto (${{indiceFoto + 1}}/${{total}})`;
      }}

      async function enviarUnaAUna() {{
        if (indiceFoto >= archivosDataFotos.length) return;
        const a = archivosDataFotos[indiceFoto];
        msgFotos.textContent = "";
        try {{
          const file = construirFile(a);
          // Se intenta compartir directamente, SIN pasar antes por
          // navigator.canShare(): en algunos dispositivos esa
          // comprobación previa da "false" de forma demasiado
          // conservadora aunque el envío real sí funcionaría, y eso
          // bloqueaba el envío sin motivo real.
          await navigator.share({{
            files: [file],
            title: "Foto del informe de detectores de Rn",
            text: textoMensajeFotos,
          }});
          indiceFoto++;
        }} catch (err) {{
          if (err.name === "AbortError") {{
            msgFotos.textContent = "Cancelado. Pulsa de nuevo para reintentar esta foto.";
          }} else if (err.name === "TypeError" || err.name === "NotSupportedError") {{
            // El navegador no admite compartir archivos: probamos solo
            // con texto para no dejar el botón sin hacer nada.
            try {{
              await navigator.share({{ title: "Foto del informe de detectores de Rn", text: textoMensajeFotos }});
            }} catch (err2) {{ /* se ignora, ya se informa abajo */ }}
            msgFotos.textContent = "Tu navegador no admite adjuntar archivos directamente.";
            indiceFoto++;
          }} else {{
            msgFotos.textContent = "No se pudo compartir esta foto: " + err.message;
            indiceFoto++;  // seguimos con la siguiente para no bloquear el resto
          }}
        }}
        actualizarUIPaso();
      }}
      ...
      btnFotos.addEventListener("click", async () => {{
        ...
        // --- Intento 1: todas las fotos juntas en un solo toque ---
        // Igual que en enviarUnaAUna(), se intenta directamente sin la
        // comprobación previa navigator.canShare(), que puede devolver
        // "false" de forma demasiado conservadora en algunos móviles
        // aunque el envío real funcionaría perfectamente.
        try {{
          const files = archivosDataFotos.map(construirFile);
          await navigator.share({{
            files: files,
            title: "Fotos del informe de detectores de Rn",
            text: textoMensajeFotos,
          }});
          msgFotos.textContent = `${{files.length}} foto(s) compartidas correctamente.`;
          return;
        }} catch (err) {{
          if (err.name === "AbortError") {{
            msgFotos.textContent = "Envío cancelado.";
            return;
          }}
          // Si falla el envío conjunto (dispositivo más limitado,
          // demasiadas fotos...), caemos al modo una a una como
          // respaldo en vez de dejar al usuario sin ninguna opción.
        }}

        modoUnaAUna = true;
        msgFotos.textContent = "El envío conjunto no está disponible en este dispositivo; se enviarán una a una.";
        indiceFoto = 0;
        actualizarUIPaso();
      }});
    </script>
...
def pantalla_inicio():
    st.title("☢️ Detectores de Radón")
    st.markdown(
        '<p style="color:#F5A623; font-size:1.1rem; font-weight:700; margin:0;">'
        'Registro de colocación</p>',
        unsafe_allow_html=True,
    )
    ...
    if st.button("➕ Nuevo centro", use_container_width=True, type="tertiary"):
        st.session_state.mostrar_form_nuevo_centro = not st.session_state.mostrar_form_nuevo_centro
        st.rerun()
    ...
    if st.button("📥 Importar centro", use_container_width=True):
        st.session_state.mostrar_form_importar = not st.session_state.mostrar_form_importar
        st.rerun()
    ...
    if st.session_state.mostrar_form_importar:
        st.caption(
            ...
            "esta misma app con «📦 Generar documentos»."
        )
        archivo_importar = st.file_uploader(
            "Selecciona el archivo .xlsx", type=["xlsx"], key="importar_centro_file",
        )
        if st.button("Importar", key="btn_confirmar_importar"):
            if not archivo_importar:
                st.warning("Selecciona primero un archivo .xlsx")
            else:
                try:
...
def pantalla_centro():
    ...
    cid, nombre, zona, fecha, img_path, tecnico_centro = centro
    ...
    with top1:
        if st.button("← Volver a Centros"):
            ...
            st.rerun()
    with top2:
        if st.button("🔧 Técnico", use_container_width=True):
    ...
    mostrar_datos_key = ns_centro + "_mostrar_datos"
    if mostrar_datos_key not in st.session_state:
        st.session_state[mostrar_datos_key] = True

    if st.button("Datos del centro", type="tertiary", use_container_width=True):
        st.session_state[mostrar_datos_key] = not st.session_state[mostrar_datos_key]
        st.rerun()

    if st.session_state[mostrar_datos_key]:
        # OJO: Streamlit borra el valor de un text_input de su
        # session_state en cuanto ese widget deja de renderizarse en
        # una ejecución (p.ej. al salir a la pantalla de inicio). Por
        # eso aquí se usa "value=" directamente con el dato de la base
        # de datos en cada entrada a la pantalla, en vez de precargarlo
        # solo la primera vez (ese patrón perdía los datos al volver).
        ...
        with c1:
            ...
            st.text_input(
                "Fecha", value=fecha or datetime.now().strftime("%d/%m/%Y"),
                key=ns_centro + "_fecha",
            )
        with c2:
            widget_imagen("Imagen exterior", img_key, key_prefix=ns_centro + "_imgw")
    ...
    st.markdown("---")
    st.subheader("🗺️ Planos del centro")
    st.caption(
        "Carga aquí los planos de las plantas del centro (hasta 3). Cada "
    ...
    if len(planos_centro) < 3:
        with st.expander("➕ Añadir plano"):
            ...
            if st.button("Guardar plano", key=f"guardar_plano_{cid}"):
        ...
        st.caption("Ya hay 3 planos cargados (el máximo). Elimina alguno si quieres añadir otro distinto.")
    ...
    st.markdown("---")
    st.subheader("📄 Informes y descargas")

    if not detectores:
        st.caption("Añade al menos un detector para poder generar los archivos.")
    else:
        # Recopilar todas las fotos del informe (imagen exterior del centro
        # y plano/situación/detector de cada detector). Se usa tanto para
        # el ZIP de descarga como para la checklist de WhatsApp.
        fotos_disponibles = []
        ...
        # Los planos se listan una sola vez cada uno (se comparten entre
        # varios detectores, así que no se repiten por cada uno de ellos).
        for plano_c in fetch_planos_centro(cid):
            _, _, nombre_plano_c, ruta_plano_c, _ = plano_c
            if ruta_plano_c and os.path.exists(ruta_plano_c):
...
def pantalla_ajustes():
    ...
    _, nombre_centro, _, _, _, tecnico_actual = centro
...
def inyectar_traduccion_widgets():
    ...
    (function() {
      const traducciones = [
        ...
        ["Limit 200MB per file", "Límite 200MB por archivo"],
      ];
