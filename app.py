...
st.markdown(
    ...
    div:has(> div.marcador-btn-generar) + div button:hover {
    ...
    /* Botones destacados en azul (Nuevo centro, Guardar centro, Nuevo
       detector, Generar documentos ya cubierto arriba). */
    div:has(> div.marcador-btn-azul) + div button {
        background-color: #2563EB !important;
        border-color: #2563EB !important;
    }
    div:has(> div.marcador-btn-azul) + div button:hover {
        background-color: #1d4ed8 !important;
        border-color: #1d4ed8 !important;
    }
...
def boton_compartir_whatsapp_fotos(imagenes, texto_mensaje):
    ...
    checklist (sin el PDF), enviándolas UNA A UNA (una petición de
    compartir por cada foto, en secuencia) en vez de todas juntas en un
    solo intento. Enviarlas todas a la vez en un único archivo (o varias
    adjuntas de golpe) falla en bastantes navegadores/Android; una a
    una es mucho más fiable.
    ...
    <div style="font-family: 'Source Sans Pro', sans-serif;">
      <button id="btn-compartir-fotos" style="
          ...
          padding:12px 20px; border-radius:8px; font-size:16px;
          font-weight:600; cursor:pointer; width:100%;">
        🖼️ Enviar {n_fotos} foto(s) por WhatsApp (una a una)
      </button>
    ...
    <script>
      ...
      btnFotos.addEventListener("click", async () => {{
        let enviadas = 0;
        for (let i = 0; i < archivosDataFotos.length; i++) {{
          const a = archivosDataFotos[i];
          try {{
            const file = new File([b64ToBlobFotos(a.b64, a.type)], a.name, {{ type: a.type }});
            if (navigator.canShare && navigator.canShare({{ files: [file] }})) {{
              msgFotos.textContent = `Compartiendo foto ${{i + 1}} de ${{archivosDataFotos.length}}...`;
              await navigator.share({{
                files: [file],
            ...
            if (err.name === "AbortError") {{
              msgFotos.textContent = `Envío cancelado (${{enviadas}} de ${{archivosDataFotos.length}} enviadas).`;
              return;
            }}
            // Si una foto concreta falla, seguimos con las siguientes en
            // vez de detener todo el envío.
          }}
        }}
        msgFotos.textContent = `Listo: ${{enviadas}} de ${{archivosDataFotos.length}} foto(s) compartidas.`;
      }});
    </script>
    """
    components.html(html, height=110)
...
def pantalla_inicio():
    ...
    st.markdown('<div class="marcador-btn-azul"></div>', unsafe_allow_html=True)
    ...
    if not detectores:
        ...
        with col_gen:
            ...
            generar_clic = st.button(
                "📦 Generar documentos", type="primary", use_container_width=True,
            )
