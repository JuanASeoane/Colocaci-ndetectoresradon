...
def generar_excel(centro_id, output_path):
    ...
    una fila con todos sus datos y sus tres fotos (plano, situación,
    detector) incrustadas cada una en su propia celda."""
    ...
    TAM_IMG_PX = 150  # tamaño (ancho y alto) de cada foto dentro de su celda

    fila = header_row + 1
    for d in detectores:
        (did, _, planta, sala, fecha_det, codigo, plano_p, px, py,
            ...
            if ruta and os.path.exists(ruta):
                try:
                    with Image.open(ruta) as im:
                        ...
                        buf.seek(0)
                    xl_img = XLImage(buf)
                    xl_img.width = TAM_IMG_PX
                    xl_img.height = TAM_IMG_PX
                    ws.add_image(xl_img, f"{col_letra[col_name]}{fila}")
                except Exception:
                    ws[f"{col_letra[col_name]}{fila}"] = "(no se pudo incrustar la imagen)"
        fila += 1
