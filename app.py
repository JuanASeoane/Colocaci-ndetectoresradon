    ...
    try:
        ...
        for idx, d in enumerate(detectores, 1):
            ...
            if plano and os.path.exists(plano):
                story.append(Paragraph("Ubicacion en el plano:", estilo_subtitulo))
                try:
    ...
    if not detectores:
        ...
        with gcol1:
            generar_clic = st.button(
                "📦 Generar archivos de descarga", type="primary", use_container_width=True,
            )
        with gcol2:
            descargar_fotos_clic = st.button(
                "🖼️ Descargar fotos", use_container_width=True, disabled=not fotos_disponibles,
            )
        ...
        if generar_clic:
            try:
                ...
                with st.spinner("Generando PDF y hoja de cálculo..."):
                ...
                st.success("PDF y Excel generados correctamente")
            except Exception as e:
                st.error(f"Error al generar los archivos: {e}")

        if descargar_fotos_clic:
            try:
                nombre_zip = f"fotos_{_slug(nombre)}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.zip"
                ...
                st.error(f"Error al preparar las fotos: {e}")

        ultimo_zip = st.session_state.get("ultimo_zip_fotos")
        if ultimo_zip and os.path.exists(ultimo_zip) and st.session_state.get("ultimo_zip_fotos_centro") == cid:
