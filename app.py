-*- coding: utf-8 -*-
"""
APP DE RADON PARA STREAMLIT - CON CLICK EN PLANO
=================================================
"""
...
def generar_pdf(centro_id, output_path):
    try:
        ...
        story.append(Paragraph("Informe de colocaci贸n de detectores de Rn", styles["Title"]))
        ...
        if tecnico:
            story.append(Paragraph(f"<b>T茅cnico:</b> {tecnico}", centrado))
        ...
        for idx, d in enumerate(detectores, 1):
            ...
            if plano and os.path.exists(plano):
                story.append(Paragraph("Ubicaci贸n en el plano:", styles["Heading4"]))
                try:
...
def init_session_state():
    ...
    if 'punto_x_temp' not in st.session_state:
        st.session_state.punto_x_temp = -1
    if 'punto_y_temp' not in st.session_state:
        st.session_state.punto_y_temp = -1
    if 'plano_temp' not in st.session_state:
        st.session_state.plano_temp = None

# ============================================================
# COMPONENTE DE PLANO CON CLICK - Usando streamlit-image-coordinates
# ============================================================

def plano_con_click(plano_path, punto_x=-1, punto_y=-1, key="plano"):
    """
    Muestra un plano donde se puede hacer clic para seleccionar la ubicaci贸n.
    Usa streamlit-image-coordinates para capturar el clic.
    """
    
    if plano_path is None or not os.path.exists(plano_path):
        st.info("馃搵 Sube un plano para poder marcar la ubicaci贸n del detector")
    ...
    try:
        ...
        # Crear una copia con el punto marcado si existe
        img_marked = img.copy()
        draw = ImageDraw.Draw(img_marked)
        
        # Dibujar el punto si existe
        if punto_x is not None and punto_x >= 0 and punto_y is not None and punto_y >= 0:
            ...
            draw.ellipse([cx - r - 4, cy - r - 4, cx + r + 4, cy + r + 4],
                        outline=(255, 200, 200), width=2)
        
        # Mostrar coordenadas actuales
        if punto_x >= 0:
            st.info(f"馃搷 Ubicaci贸n seleccionada: X={punto_x:.3f}, Y={punto_y:.3f}")
        else:
            st.warning("鈿狅笍 Haz clic en la imagen para marcar la ubicaci贸n del detector")
        
        # Mostrar la imagen con el componente interactivo
        st.caption("馃憜 Haz clic en la imagen para marcar la ubicaci贸n del detector")
        
        # Usar streamlit-image-coordinates para capturar clic
        # El valor retornado es un dict con 'x' e 'y' en p铆xeles
        result = streamlit_image_coordinates(img_marked, key=f"{key}_click")
        
        # Si hay un clic, convertir coordenadas a fracci贸n (0-1)
        if result is not None:
            click_x = result['x'] / w
            click_y = result['y'] / h
            
            # Limitar a [0, 1]
            click_x = max(0, min(1, click_x))
            click_y = max(0, min(1, click_y))
        ...
        with col1:
            if st.button("馃幆 Centrar", key=f"{key}_center"):
                punto_x, punto_y = 0.5, 0.5
                st.rerun()
        with col2:
            if st.button("馃棏锔?Quitar punto", key=f"{key}_clear"):
                punto_x, punto_y = -1, -1
                st.rerun()
        with col3:
            if st.button("馃攧 Recargar", key=f"{key}_reload"):
                st.rerun()
...
def page_inicio():
    st.title("馃彚 Centros Registrados")
    ...
    with col2:
        if st.button("馃搨 Abrir", use_container_width=True):
            if st.session_state.selected_centro_id:
                ...
                st.rerun()
    with col3:
        if st.button("鉃?Nuevo", use_container_width=True):
            st.session_state.page = 'nuevo_centro'
            st.rerun()
    with col4:
        if st.button("鈿欙笍 Ajustes", use_container_width=True):
    ...
    if centros:
        ...
        with col2:
            if st.button("馃棏锔?Eliminar", use_container_width=True):
                if st.session_state.selected_centro_id:
...
# ============================================================
# P脕GINA DE NUEVO CENTRO
# ============================================================

def page_nuevo_centro():
    st.title("鉃?Nuevo Centro")
    
    with st.form("nuevo_centro_form"):
        nombre = st.text_input("Nombre del centro *")
        zona = st.text_input("Zona")
        fecha = st.date_input("Fecha de medici贸n", datetime.now())
        
        st.subheader("馃摲 Imagen exterior")
        
        uso_camara = st.checkbox("馃摳 Usar c谩mara", value=False)
        
        if uso_camara:
            foto_camara = st.camera_input("Tomar foto con la c谩mara")
            if foto_camara:
...
def page_centro():
    ...
    with col1:
        st.title(f"馃彚 {nombre}")
    with col2:
        if st.button("鈫?Volver", use_container_width=True):
            st.session_state.page = 'inicio'
            st.rerun()
    
    with st.expander("馃搵 Datos del Centro", expanded=True):
    ...
    with st.expander("鉁忥笍 Editar Centro"):
        with st.form("editar_centro_form"):
            ...
            uso_camara_editar = st.checkbox("馃摳 Usar c谩mara", key="editar_camara")
    ...
    st.subheader("馃搳 Detectores")
    
    detectores = fetch_detectores(cid)
    
    col1, col2, col3 = st.columns([1, 1, 1])
    with col1:
        if st.button("鉃?Nuevo Detector", use_container_width=True):
            st.session_state.detector_actual = None
            st.session_state.editando_detector = False
            st.session_state.page = 'detector'
            st.rerun()
    with col2:
        if st.button("馃搫 Generar PDF", use_container_width=True):
            if detectores:
                try:
                    ...
                    if os.path.exists(pdf_path):
                        st.success("鉁?PDF generado correctamente")
                        ...
                        st.error("鉂?Error: El PDF no se gener贸 correctamente")
                        
                except Exception as e:
                    st.error(f"鉂?Error al generar PDF: {str(e)}")
            else:
                st.warning("鈿狅笍 No hay detectores para generar el informe")
    ...
    if detectores:
        ...
        col1, col2 = st.columns(2)
        with col1:
            if st.button("鉁忥笍 Editar", use_container_width=True):
...
# ============================================================
# P脕GINA DE DETECTOR CON CLICK EN PLANO
# ============================================================

def page_detector():
    st.title("馃搶 Detector")
    ...
    if st.session_state.detector_actual and st.session_state.editando_detector:
        ...
        if d:
            ...
            es_edicion = True
            if st.session_state.plano_temp is None:
                st.session_state.plano_temp = plano
            if st.session_state.punto_x_temp == -1:
                st.session_state.punto_x_temp = px if px is not None and px >= 0 else -1
            if st.session_state.punto_y_temp == -1:
                st.session_state.punto_y_temp = py if py is not None and py >= 0 else -1
        else:
    ...
    # --- PLANO CON CLICK (fuera del form) ---
    st.subheader("馃搻 Plano y Ubicaci贸n")
    st.caption("馃憜 **Haz clic directamente en la imagen** para marcar la ubicaci贸n del detector")
    
    # Subir nuevo plano
    plano_file = st.file_uploader("Subir plano", type=['png', 'jpg', 'jpeg'], key="plano_upload")
    if plano_file:
        st.image(plano_file, width=400)
        plano_temp_path = os.path.join(get_data_dir(), f"plano_temp_{datetime.now().strftime('%Y%m%d_%H%M%S')}.jpg")
        with open(plano_temp_path, "wb") as f:
            f.write(plano_file.getbuffer())
        st.session_state.plano_temp = plano_temp_path
        st.rerun()
    
    # Mostrar plano con selector de clic
    plano_actual = st.session_state.plano_temp if st.session_state.plano_temp else plano
    
    if plano_actual and os.path.exists(plano_actual):
        nuevo_px, nuevo_py = plano_con_click(
            plano_actual, 
            st.session_state.punto_x_temp, 
            st.session_state.punto_y_temp,
            key="plano_click"
        )
        st.session_state.punto_x_temp = nuevo_px
        st.session_state.punto_y_temp = nuevo_py
    else:
        st.info("馃搵 Sube un plano para poder marcar la ubicaci贸n del detector")
    
    # Bot贸n para eliminar plano (fuera del form)
    if st.button("馃棏锔?Eliminar plano", key="delete_plano"):
        st.session_state.plano_temp = None
        st.session_state.punto_x_temp = -1
        st.session_state.punto_y_temp = -1
        st.rerun()
    ...
    with st.form("detector_form"):
        ...
        with col1:
            planta_input = st.text_input("Planta", value=planta)
            sala_input = st.text_input("Sala *", value=sala)
            fecha_input = st.date_input("Fecha", 
                                       value=datetime.strptime(fecha, "%d/%m/%Y") if fecha else datetime.now())
            codigo_input = st.text_input("C贸digo *", value=codigo)
        
        with col2:
            st.subheader("馃摲 Foto de Situaci贸n")
            uso_camara_sit = st.checkbox("馃摳 Usar c谩mara", key="camara_sit")
            ...
            if uso_camara_sit:
                ...
                elif foto_sit and os.path.exists(foto_sit):
                    st.caption("Actual:")
                    mostrar_imagen(foto_sit, width=150)
            
            st.subheader("馃摲 Foto del Detector")
            uso_camara_det = st.checkbox("馃摳 Usar c谩mara", key="camara_det")
            ...
            if uso_camara_det:
                ...
                elif foto_det and os.path.exists(foto_det):
                    st.caption("Actual:")
        ...
        col1, col2 = st.columns(2)
        with col1:
            if st.session_state.punto_x_temp >= 0:
                st.metric("馃搷 Posici贸n X", f"{st.session_state.punto_x_temp:.3f}")
            else:
                st.warning("鈿狅笍 Sin ubicaci贸n seleccionada")
        with col2:
            if st.session_state.punto_y_temp >= 0:
                st.metric("馃搷 Posici贸n Y", f"{st.session_state.punto_y_temp:.3f}")
            else:
                st.warning("鈿狅笍 Sin ubicaci贸n seleccionada")
        ...
        col1, col2 = st.columns(2)
        with col1:
            if st.form_submit_button("馃捑 Guardar Detector", use_container_width=True):
                if not sala_input.strip():
                    st.error("La sala es obligatoria")
                elif not codigo_input.strip():
                    st.error("El c贸digo es obligatorio")
                else:
                    sit_final = None
                    det_final = None
                    plano_final = st.session_state.plano_temp if st.session_state.plano_temp else plano
                    ...
                    elif foto_sit and isinstance(foto_sit, str) and os.path.exists(foto_sit):
                        sit_final = foto_sit
                    ...
                    elif foto_det and isinstance(foto_det, str) and os.path.exists(foto_det):
                        det_final = foto_det
                    
                    if plano_file:
                        plano_final = guardar_imagen_subida(plano_file, "plano")
...
def page_ajustes():
    st.title("鈿欙笍 Ajustes")
    
    tecnico_actual = get_tecnico()
    
    with st.form("ajustes_form"):
        tecnico = st.text_input("T茅cnico / Empresa (aparece en el PDF)", value=tecnico_actual)
