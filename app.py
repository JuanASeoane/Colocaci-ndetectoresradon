...
st.markdown(
    ...
    /* Botones destacados (type="primary") en verde en vez del rojo por defecto */
...
def generar_pdf(centro_id, output_path):
    try:
        ...
        if tecnico:
            story.append(Paragraph(f"<b>Tecnico:</b> {tecnico}", centrado))
...
def boton_compartir_whatsapp_completo(pdf_path, nombre_pdf, imagenes, texto_mensaje):
    """Botón que comparte por WhatsApp el PDF Y además todas las fotos
    del informe (plano y fotos de cada detector, imagen exterior del
    centro) en un mismo envío, usando la Web Share API con varios
    archivos a la vez.
    ...
    n_fotos = len(archivos) - 1

    html = f"""
    <div style="font-family: 'Source Sans Pro', sans-serif;">
      <button id="btn-compartir-todo" style="
          background-color:#25D366; color:white; border:none;
          padding:12px 20px; border-radius:8px; font-size:16px;
          font-weight:600; cursor:pointer; width:100%;">
        📲🖼️ Enviar informe completo (PDF + {n_fotos} fotos) por WhatsApp
      </button>
      <p id="msg-compartir-todo" style="margin-top:8px; font-size:13px; color:#666;"></p>
    </div>
    <script>
      const archivosDataTodo = {archivos_json};
      const textoMensajeTodo = {texto_json};

      function b64ToBlobTodo(b64Data, contentType) {{
        const byteChars = atob(b64Data);
        const byteNumbers = new Array(byteChars.length);
        for (let i = 0; i < byteChars.length; i++) {{
          byteNumbers[i] = byteChars.charCodeAt(i);
      ...
      const btnTodo = document.getElementById("btn-compartir-todo");
      const msgTodo = document.getElementById("msg-compartir-todo");

      btnTodo.addEventListener("click", async () => {{
        try {{
          const files = archivosDataTodo.map(
            a => new File([b64ToBlobTodo(a.b64, a.type)], a.name, {{ type: a.type }})
          );
          if (navigator.canShare && navigator.canShare({{ files }})) {{
            await navigator.share({{
              files: files,
              title: "Informe de detectores de Rn",
              text: textoMensajeTodo,
            }});
          }} else if (navigator.share) {{
            await navigator.share({{ title: "Informe de detectores de Rn", text: textoMensajeTodo }});
            msgTodo.textContent = "Tu navegador no admite adjuntar varios archivos a la vez; comparte el PDF y las fotos por separado.";
          }} else {{
            msgTodo.textContent = "Tu navegador no soporta compartir archivos. Descarga el PDF y las fotos manualmente.";
          }}
        }} catch (err) {{
          if (err.name !== "AbortError") {{
            msgTodo.textContent = "No se pudo abrir el diálogo de compartir: " + err.message;
          }}
        }}
      }});
    </script>
...
def pantalla_inicio():
    ...
    with col_t:
        ...
        st.caption("Centros registrados")
    with col_a:
...
def pantalla_centro():
    ...
    with st.expander("Datos del centro", expanded=True):
        ...
        with c1:
            ...
            st.text_input("Zona", key=ns_centro + "_zona")
    ...
    if not detectores:
        ...
        if ultimo_pdf and os.path.exists(ultimo_pdf) and st.session_state.get("ultimo_pdf_centro") == cid:
            ...
            if fotos_disponibles:
                st.markdown("**Adjuntar también estas fotos del informe:**")
                seleccionadas = []
                ...
                texto_wa_completo = (
                    f"Informe completo de colocación de detectores de Rn – {nombre or ''}"
                )
                boton_compartir_whatsapp_completo(
                    ultimo_pdf,
                    st.session_state.get("ultimo_pdf_nombre", "informe.pdf"),
                    seleccionadas,
                    texto_wa_completo,
