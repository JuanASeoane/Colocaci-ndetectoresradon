...
st.markdown(
    ...
    div[data-testid="stElementContainer"]:has(div.marcador-num-personas) + div[data-testid="stElementContainer"] div[data-testid="stNumberInput"] {
...
def pantalla_centro():
    ...
    if _cabecera_bloque("Categorías profesionales", "categorias"):
        with st.container(border=True):
            ...
            cnew1, cnew2, cnew3 = st.columns([3, 1, 1.3])
            with cnew1:
            ...
            with cnew2:
                ...
                nuevo_num_personas = st.number_input(
                    ...
                    min_value=0, max_value=99, step=1, label_visibility="collapsed",
                )
            with cnew3:
            ...
            if categorias_centro:
                for cat_id, _, categoria, num_personas in categorias_centro:
                    ccat1, ccat2 = st.columns([4, 1.6])
                    with ccat1:
                        st.write(f"{categoria}: {num_personas}")
                    with ccat2:
                        st.markdown('<div class="marcador-btn-eliminar"></div>', unsafe_allow_html=True)
                        if st.button("❌ Eliminar", key=f"del_cat_{cat_id}"):
