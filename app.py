    ...
    if con_camara:
        with tabs[1]:
            foto = st.camera_input(
                "Capturar foto", key=key_prefix + "_cam", label_visibility="collapsed",
            )
            if foto is not None:
                fid = ("cam", getattr(foto, "file_id", None) or f"len{len(foto.getvalue())}")
                if st.session_state.get(file_id_key) != fid:
...
def widget_plano(state_prefix):
    ...
    with tabs[1]:
        foto = st.camera_input(
            "Capturar plano", key=state_prefix + "_cam", label_visibility="collapsed",
        )
        if foto is not None:
            fid = ("cam", getattr(foto, "file_id", None) or f"len{len(foto.getvalue())}")
            if st.session_state.get(file_id_key) != fid:
...
def pantalla_inicio():
    ...
    st.markdown("---")
    for c in centros:
        cid, nombre, zona, fecha, img_path = c
        with st.container(border=True):
            cols = st.columns([1, 3, 2, 2, 1.3, 1.3])
            with cols[0]:
            ...
            with cols[3]:
                st.write(fecha or "—")
            with cols[4]:
                if st.button("Abrir", key=f"abrir_{cid}", use_container_width=True):
                    st.session_state.centro_actual = cid
                    st.session_state.view = "centro"
                    st.rerun()
            with cols[5]:
                if st.button("🗑️", key=f"del_{cid}", use_container_width=True):
                    st.session_state["confirmar_borrado_centro"] = cid
                    st.rerun()

            if st.session_state.get("confirmar_borrado_centro") == cid:
                st.warning(f"¿Eliminar el centro «{nombre}» y todos sus detectores? Esta acción no se puede deshacer.")
                cc1, cc2 = st.columns(2)
                with cc1:
                    if st.button("Sí, eliminar", key=f"confirmar_del_{cid}", type="primary"):
                        delete_centro(cid)
                        st.session_state["confirmar_borrado_centro"] = None
                        st.rerun()
                with cc2:
                    if st.button("Cancelar", key=f"cancelar_del_{cid}"):
                        st.session_state["confirmar_borrado_centro"] = None
                        st.rerun()
