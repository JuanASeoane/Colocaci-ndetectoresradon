...
def plano_con_click(plano_path, punto_x=-1, punto_y=-1, key="plano"):
    """
    Muestra un plano donde se puede hacer clic para seleccionar la ubicación.
    ...
    try:
        ...
        if punto_x >= 0:
            st.info(f"📍 Ubicación seleccionada: X={punto_x:.3f}, Y={punto_y:.3f}")
        else:
            st.warning("⚠️ Haz clic en la imagen para marcar la ubicación del detector")
        
        # Mostrar la imagen con el componente interactivo
        st.caption("👆 Haz clic en la imagen para marcar la ubicación del detector")
        
        # Usar streamlit-image-coordinates para capturar clic
        # El valor retornado es un dict con 'x' e 'y' en píxeles
        result = streamlit_image_coordinates(img_marked, key=f"{key}_click")
        
        # Si hay un clic, convertir coordenadas a fracción (0-1)
        ...
        with col1:
            if st.button("🎯 Centrar", key=f"{key}_center"):
                punto_x, punto_y = 0.5, 0.5
                st.rerun()
        with col2:
            if st.button("🗑️ Quitar punto", key=f"{key}_clear"):
                punto_x, punto_y = -1, -1
                st.rerun()
        with col3:
            if st.button("🔄 Recargar", key=f"{key}_reload"):
                st.rerun()
...
# ============================================================
# PÁGINA DE DETECTOR CON CLICK EN PLANO
# ============================================================
...
def page_detector():
    ...
    st.caption("👆 **Haz clic directamente en la imagen** para marcar la ubicación del detector")
    ...
    # Botón para eliminar plano (fuera del form)
    ...
    with st.form("detector_form"):
        ...
        with col1:
            if st.session_state.punto_x_temp >= 0:
                st.metric("📍 Posición X", f"{st.session_state.punto_x_temp:.3f}")
            else:
                st.warning("⚠️ Sin ubicación seleccionada")
        with col2:
            if st.session_state.punto_y_temp >= 0:
                st.metric("📍 Posición Y", f"{st.session_state.punto_y_temp:.3f}")
            else:
                st.warning("⚠️ Sin ubicación seleccionada")
