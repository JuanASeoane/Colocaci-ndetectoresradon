...
def generar_pdf(centro_id, output_path):
    try:
        ...
        if categorias_centro_pdf:
            story.append(Paragraph("<b>Categorías profesionales expuestas</b>", centrado))
...
def generar_excel(centro_id, output_path):
    ...
    fuente_naranja_dato = Font(name=FUENTE, size=8, color="F5A623")
...
def pantalla_centro():
    ...
    if st.session_state[mostrar_datos_key]:
        ...
        st.markdown(
            ...
            '👷 Categorías profesionales expuestas</p>',
            unsafe_allow_html=True,
        )
        ...
        if categorias_centro:
            for cat_id, _, categoria, num_personas in categorias_centro:
                ccat1, ccat2, ccat3 = st.columns([3, 2, 1])
                with ccat1:
                    st.write(categoria)
                with ccat2:
                    st.write(f"{num_personas} persona(s)")
                with ccat3:
                    st.markdown('<div class="marcador-btn-eliminar"></div>', unsafe_allow_html=True)
                    if st.button("❌", key=f"del_cat_{cat_id}"):
                        delete_categoria_centro(cat_id)
                        st.rerun()
        else:
            st.caption("Todavía no se ha añadido ninguna categoría profesional.")

        cnew1, cnew2, cnew3 = st.columns([3, 2, 1])
        with cnew1:
            nueva_categoria = st.text_input(
                "Categoría profesional", key=f"nueva_categoria_nombre_{cid}",
                label_visibility="collapsed", placeholder="Categoría profesional",
            )
        with cnew2:
            nuevo_num_personas = st.number_input(
                "Nº personas expuestas", key=f"nueva_categoria_num_{cid}",
                min_value=0, step=1, label_visibility="collapsed",
            )
        with cnew3:
            if st.button("➕ Añadir", key=f"add_categoria_{cid}"):
