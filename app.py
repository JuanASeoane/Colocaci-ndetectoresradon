...
def pantalla_centro():
    ...
    if not detectores:
        ...
        did_sel, _, planta_sel, sala_sel, fecha_sel_det, codigo_sel = (
            d_sel[0], d_sel[1], d_sel[2], d_sel[3], d_sel[4], d_sel[5]
        )

        with st.container(border=True):
            info_cols = st.columns(4)
            info_cols[0].markdown(f"**Planta**\n\n{planta_sel or '—'}")
            info_cols[1].markdown(f"**Sala**\n\n{sala_sel or '—'}")
            info_cols[2].markdown(f"**Código**\n\n{codigo_sel or '—'}")
            info_cols[3].markdown(f"**Fecha**\n\n{fecha_sel_det or '—'}")
