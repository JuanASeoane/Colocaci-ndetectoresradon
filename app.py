...
def widget_imagen(label, state_key, key_prefix, con_camara=True):
    ...
    if con_camara:
        with tabs[1]:
            # La key incluye un "nonce": en cuanto se procesa una foto,
            # el nonce cambia y Streamlit crea un widget de cámara nuevo
            # y vacío, para que la miniatura capturada no se quede
            # duplicada junto a la vista previa propia de la app.
...
def widget_plano(state_prefix):
    ...
    with tabs[1]:
        # Igual que en widget_imagen: la key lleva un "nonce" que cambia
        # tras cada captura para que el widget de cámara se vacíe y no
        # se quede la miniatura duplicada junto al plano ya guardado.
