...
def insert_detector(data):
    ...
    c.execute('''INSERT INTO detectores
                 (centro_id, planta, sala, fecha, detector_codigo, plano_path,
                  ...
                  codigo_sala, profesionales_sala)
                 VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?)''', data)
    rowid = c.lastrowid
    conn.commit()
    conn.close()
    ...
    try:
        ...
        for idx, d in enumerate(detectores, 1):
            (did, _, planta, sala, fecha_det, codigo, plano, px, py, foto_sit, foto_det, _,
             codigo_sala, profesionales_sala) = d
...
def generar_excel(centro_id, output_path):
    ...
    # --- Tabla plana: nada por encima de la cabecera. "Centro" y "Área"
    # son las dos primeras columnas de cada fila, para poder pegar
    # debajo los datos de otros centros sin que estorbe ningún bloque. ---
    header_row = 1
    headers = [
        "Centro", "Área", "ID", "Planta", "Sala", "Código de la sala",
        "Profesionales en la sala", "Código",
        "Fecha de colocación", "Fecha de retirada óptima", "Fecha de retirada real",
    ...
    for d in detectores:
        (did, _, planta, sala, fecha_det, codigo, plano_p, punto_x, punto_y,
         foto_sit_p, foto_det_p, fecha_creacion, codigo_sala, profesionales_sala) = d

        fecha_retirada_optima = calcular_fecha_retirada_optima(fecha_det)

        valores = [
            nombre or "", zona or "", did, planta or "", sala or "",
            codigo_sala or "", profesionales_sala or "", codigo or "",
            fecha_det or "", fecha_retirada_optima, "",
        ]
...
def pantalla_detector():
    ...
    if not st.session_state.get(init_key):
        if detector_id:
            ...
            planta = sala = codigo = codigo_sala = profesionales_sala = ""
    ...
    st.markdown("---")
    st.subheader("Fotos (lado a lado)")
