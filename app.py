# -*- coding: utf-8 -*-
"""
APP DE RADON PARA STREAMLIT - CON SLIDERS VISIBLES
===================================================
"""
...
def init_session_state():
    ...
    # Variables para el plano - con valores por defecto
    ...
    if 'plano_cargado' not in st.session_state:
        st.session_state.plano_cargado = False
            ...
            if st.button("✏️ Editar", use_container_width=True):
                if st.session_state.detector_actual:
                    ...
                    if d:
                        ...
                        st.session_state.plano_cargado = True if d[6] and os.path.exists(d[6]) else False
...
# ============================================================
# PÁGINA DE DETECTOR CON SLIDERS - VERSIÓN CORREGIDA
# ============================================================
...
def page_detector():
    ...
    if st.session_state.detector_actual and st.session_state.editando_detector:
        ...
        if d:
            ...
            es_edicion = True
            # Si no hay plano en session_state, usar el de la BD
            if st.session_state.plano_path is None:
                st.session_state.plano_path = plano
                st.session_state.plano_cargado = True if plano and os.path.exists(plano) else False
    ...
    if plano_file:
        ...
        if plano_path:
            ...
            st.session_state.plano_cargado = True
            st.success("✅ Plano subido correctamente")
            st.rerun()
    
    # Verificar si hay un plano cargado
    plano_actual = st.session_state.plano_path
    plano_existe = plano_actual and os.path.exists(plano_actual)
    
    # Mostrar el plano SIEMPRE que exista
    if plano_existe:
        # Mostrar el plano con el punto rojo
        ...
        if img_con_punto:
            ...
            st.error("Error al cargar el plano")
        
        # --- SLIDERS - SIEMPRE VISIBLES CUANDO HAY PLANO ---
    ...
    with st.form("detector_form"):
        ...
        with col1:
            ...
            fecha_input = st.date_input(
                "Fecha", 
                value=datetime.strptime(fecha, "%d/%m/%Y") if fecha and fecha != "" else datetime.now()
            )
