...
def widget_imagen(label, state_key, key_prefix, con_camara=True):
    ...
    if cam_activa_key not in st.session_state:
        st.session_state[cam_activa_key] = True  # sin foto todavía: cámara lista para disparar
    ...
    if con_camara:
        with tabs[1]:
            ...
            if hay_imagen and not st.session_state[cam_activa_key]:
                # Ya hay foto guardada: NO se muestra el visor de la
                # cámara, solo un botón para volver a activarlo.
                st.caption("Foto capturada (se muestra abajo).")
                if st.button("📷 Tomar otra foto", key=key_prefix + "_reactivar_cam"):
    ...
    if nuevo_bytes is not None:
        ...
        if vino_de_camara:
            # Ocultamos el visor de la cámara y preparamos una key nueva
            # para la próxima vez que el usuario pulse "Tomar otra foto".
...
def widget_plano(state_prefix):
    ...
    with tabs[1]:
        ...
        if hay_imagen and not st.session_state[cam_activa_key]:
            # Ya hay plano capturado: NO se muestra el visor de la
            # cámara, solo un botón para volver a activarlo.
            st.caption("Plano capturado (se muestra abajo).")
            if st.button("📷 Tomar otra foto", key=state_prefix + "_reactivar_cam"):
                st.session_state[cam_activa_key] = True
                st.rerun()
        else:
...
def pantalla_inicio():
    ...
    with st.container(border=True):
        prev_cols = st.columns([1, 3])
        with prev_cols[0]:
            if img_sel and os.path.exists(img_sel):
                st.image(img_sel, use_container_width=True)
        with prev_cols[1]:
            st.markdown(f"**{nombre_sel or '(sin nombre)'}**")
            st.write(f"Zona: {zona_sel or '—'}")
            st.write(f"Fecha: {fecha_sel or '—'}")
...
def pantalla_centro():
    ...
    if not detectores:
        ...
        head = st.columns([0.8, 1.5, 1.5, 1.5, 1.5, 1, 0.6])
        for h, t in zip(head, ["ID", "Planta", "Sala", "Código", "Fecha", "", ""]):
            h.markdown(f"**{t}**")
        for d in detectores:
            did, _, planta, sala, fecha_det, codigo = d[0], d[1], d[2], d[3], d[4], d[5]
            cols = st.columns([0.8, 1.5, 1.5, 1.5, 1.5, 1, 0.6])
            cols[0].write(did)
            cols[1].write(planta or "—")
            cols[2].write(sala or "—")
            cols[3].write(codigo or "—")
            cols[4].write(fecha_det or "—")
            if cols[5].button("✏️ Editar", key=f"edit_det_{did}"):
                abrir_detector_editar(did)
                st.rerun()
            if cols[6].button("🗑️", key=f"del_det_{did}"):
                st.session_state["confirmar_borrado_det"] = did
                st.rerun()

            if st.session_state.get("confirmar_borrado_det") == did:
                st.warning("¿Eliminar este detector?")
                cc1, cc2 = st.columns(2)
                if cc1.button("Sí, eliminar", key=f"conf_del_det_{did}", type="primary"):
                    delete_detector(did)
                    st.session_state["confirmar_borrado_det"] = None
                    st.rerun()
                if cc2.button("Cancelar", key=f"cancel_del_det_{did}"):
                    st.session_state["confirmar_borrado_det"] = None
                    st.rerun()
