...
def insert_detector(data):
    ...
    c.execute('''INSERT INTO detectores
                 (centro_id, planta, sala, fecha, detector_codigo, plano_path,
                  ...
                  codigo_sala, profesionales_sala, hora_colocacion)
                 VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?)''', data)
    rowid = c.lastrowid
    conn.commit()
    conn.close()
...
def generar_pdf(centro_id, output_path):
    try:
        ...
        if tecnico:
            story.append(Paragraph(f"<b>Responsable:</b> {tecnico}", centrado))
        ...
        for idx, d in enumerate(detectores, 1):
            (did, _, planta, sala, fecha_det, codigo, plano, px, py, foto_sit, foto_det, _,
             codigo_sala, profesionales_sala, hora_colocacion) = d
            ...
            tabla = Table([
                ...
                ["Sala", sala or "-"],
                ["Codigo", codigo or "-"],
                ["Codigo de la sala", codigo_sala or "-"],
                ["Profesionales en la sala", profesionales_sala or "-"],
                ["Fecha", fecha_det or "-"],
                ["Fecha optima de retirada", fecha_retirada_optima_pdf or "-"],
                ["Fecha real de retirada", ""],
...
def generar_excel(centro_id, output_path):
    ...
    headers = [
        ...
        "Profesionales en la sala", "Código",
    ...
    # Línea superior: Centro / Área / Día / Técnico
    info_pares = [
        ("Centro:", nombre or ""), ("Área:", zona or ""),
        ("Día:", fecha or ""), ("Técnico:", tecnico or ""),
    ]
    col_info = 1
    for etiqueta, valor in info_pares:
        ...
        c_etq.alignment = Alignment(horizontal="right", vertical="center")
    ...
    for h in ("Centro", "Planta", "Sala", "Código de la sala", "Profesionales en la sala", "Código"):
    ...
    for d in detectores:
        ...
        valores = [
            ...
            codigo_sala or "", profesionales_multilinea, codigo or "",
...
def pantalla_detector():
    ...
    if not st.session_state.get(init_key):
        if detector_id:
            ...
            planta = sala = codigo = codigo_sala = profesionales_sala = hora_colocacion = ""
