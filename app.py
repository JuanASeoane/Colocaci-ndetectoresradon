...
st.markdown(
    ...
    /* Botón "Eliminar" (marcado con un div justo antes, ver código
       Python): mucho más pequeño y en un naranja muy tenue/apagado,
       para que quede claramente secundario frente a "Abrir". */
    div:has(> div.marcador-btn-eliminar) + div button {
        background-color: #a8825f !important;
        border-color: #a8825f !important;
...
def pantalla_inicio():
    ...
    with b1:
        ...
        if st.button("❌ Eliminar", key="btn_eliminar_centro_home", use_container_width=True):
...
def pantalla_centro():
    ...
    if not detectores:
        ...
        with b1:
            st.markdown('<div class="marcador-btn-eliminar"></div>', unsafe_allow_html=True)
            if st.button("❌ Eliminar", key="btn_eliminar_detector_home", use_container_width=True):
