...
def insert_detector(data):
    ...
    c.execute('''INSERT INTO detectores
                 (centro_id, planta, sala, fecha, detector_codigo, plano_path,
                  ...
                  codigo_sala, profesionales_sala, hora_colocacion, turno_trabajo)
                 VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)''', data)
    rowid = c.lastrowid
    conn.commit()
    conn.close()
    ...
    try:
        ...
        for idx, d in enumerate(detectores, 1):
            (did, _, planta, sala, fecha_det, codigo, plano, px, py, foto_sit, foto_det, _,
             codigo_sala, profesionales_sala, hora_colocacion, turno_trabajo) = d
            ...
            tabla = Table([
                ["Planta", planta or "-", "Sala", sala or "-"],
                ["Codigo", codigo or "-", "Codigo de la sala", codigo_sala or "-"],
                ["Profesionales en la sala", profesionales_sala or "-", "Fecha", fecha_det or "-"],
                ["Fecha optima de retirada", fecha_retirada_optima_pdf or "-", "Fecha real de retirada", ""],
                ["Turno de trabajo", turno_trabajo or "-", "", ""],
            ], colWidths=[3.3*cm, 4.7*cm, 3.3*cm, 4.7*cm])
...
def generar_excel(centro_id, output_path):
    ...
    header_row = 2
    headers = [
        "Centro", "Área", "ID", "Planta", "Sala", "Código de la sala",
    ...
    for h in ("Centro", "Planta", "Sala", "Código de la sala", "Profesionales en la sala",
    ...
    for d in detectores:
        (did, _, planta, sala, fecha_det, codigo, plano_p, punto_x, punto_y,
         ...
         hora_colocacion, turno_trabajo) = d
        ...
        valores = [
            nombre or "", zona or "", did, planta or "", sala or "",
...
def pantalla_detector():
    ...
    if not st.session_state.get(init_key):
        if detector_id:
            ...
            planta = sala = codigo = codigo_sala = profesionales_sala = hora_colocacion = turno_trabajo = ""
