...
LOGO_PNG_B64 = (
    ...
    "OyA3wzwbS/stAIhkAMeOxZQgZU5sBpCgQrGJSJi1p74dIH6LhpM1Tnr6sUvt3OdZWIcw1oSd/vl3YbRsMpgQ85vt38sA8SXn88TiyzGQz28FiJlHvHnr1TVo8aQeMtbY9d5dHwSCHdAKJHfboafoYx5l9b5gK6N/gyymTCmDv6K7Zk1SMy6cupqC6KYIECg5pLPmAf45"
    ...
    "vdw+wHwltigAmS+Lq07Oq9lLqTmmAuSHA0QFgGBEWeCtBuO2ZGFYijXXpvFOQXQTr4vLAMG2vG0CEFCJjQxrs7pgTlwACNW2rQBZCdAInMtO5wAR0oeCGrkCkDFUNPvmwYOhsNjluwEC5lnfrw4vQDfUnxWAbH/DdLWH2a8DSMFrXQLIMvWxs4sKwpL6qAD52QAZ60A6"
    ...
    "SGhk6VyPBUh0nRXaqz1f9i7kkQuuAHkxgLC8tg9TLoJWZ6IrE2SarL1oTrIcuoe5pgLb93BiclotCglDShgYJ18FCO2j/e7kowLkTQCSemHR9V5YV4q2vhyKnJ/ZC6tKBcjPAMgw7wY4zT0aZPneGJspDWw+L6RQSNhY01GD/dTAC5D98KrLB0qFgkQonxBfBAgnvW8k"
    ...
    "WKAUBiDgFEgPj9i2te6GoqDmET9eJQNsGpx7dISpvk5xO60OhOxNtAYs6nL3reF5LRhfUfEDToAabQt0QrOsBYIY0vuFoAGBgAMD40w8QKBPGTYLiJByP1iHLxYIEPhYXUc7hw0D8SX3sX0vNPgCxKtaIF4xLW0JnFW4XluaDgKZnevEng4QDZGK6HB1Kv/EZgQBbQqu"
...
def generar_pdf(centro_id, output_path):
    try:
        ...
        for idx, d in enumerate(detectores, 1):
            (did, _, planta, sala, fecha_det, codigo, plano, px, py, foto_sit, foto_det, _,
             codigo_sala, profesionales_sala, hora_colocacion, turno_trabajo, nivel) = d
            ...
            fecha_y_hora_colocacion = fecha_det or "-"
            if hora_colocacion:
                fecha_y_hora_colocacion += f"  {hora_colocacion}"

            filas_texto = [
                ["Codigo del detector", codigo or "-", "Fecha y hora de colocacion", fecha_y_hora_colocacion],
                ["Planta", planta or "-", "Nivel", nivel or "-"],
                ["Sala", sala or "-", "Codigo de la sala", codigo_sala or "-"],
                ["Profesionales en la sala", profesionales_sala or "-", "Turno de trabajo", turno_trabajo or "-"],
                ["Fecha optima retirada", fecha_retirada_optima_pdf or "-", "Fecha real de retirada", ""],
            ]
            filas_pdf = []
...
def generar_excel(centro_id, output_path):
    ...
    fuente_cabecera = Font(name=FUENTE, size=10, bold=True, color="FFFFFF")
    ...
    # Línea superior: Centro / Área / Técnico
    info_pares = [
        ("Centro:", nombre or ""), ("Área:", zona or ""), ("Técnico:", tecnico or ""),
    ]
    col_info = 1
    for etiqueta, valor in info_pares:
        c_etq = ws.cell(row=fila_info, column=col_info, value=etiqueta)
        c_etq.font = fuente_cabecera
        c_etq.fill = PatternFill("solid", fgColor="F5A623")
        c_etq.alignment = centrado
        c_val = ws.cell(row=fila_info, column=col_info + 1, value=valor)
        c_val.font = fuente_cabecera
        c_val.fill = PatternFill("solid", fgColor="F5A623")
        c_val.alignment = centrado
        col_info += 2
    ...
    for col in range(col_info, len(headers) + 1):
    ...
    ws.row_dimensions[fila_info].height = 20
    ...
    fila = header_row + 1
    for d in detectores:
        (did, _, planta, sala, fecha_det, codigo, plano_p, punto_x, punto_y,
         foto_sit_p, foto_det_p, fecha_creacion, codigo_sala, profesionales_sala,
         hora_colocacion, turno_trabajo, nivel) = d
        ...
        hay_punto = (
            punto_x is not None and punto_y is not None
            and 0 <= punto_x <= 1 and 0 <= punto_y <= 1
        )
...
def widget_imagen(label, state_key, key_prefix, con_camara=True, con_boton_quitar=True):
...
def widget_plano(state_prefix):
    ...
    Solo admite subir un archivo (sin opción de cámara para el plano).
    """
...
# ============================================================
# COMPARTIR POR WHATSAPP - VERSIÓN MEJORADA (Solución 1)
# ============================================================

def boton_compartir_whatsapp(pdf_path, nombre_archivo, texto_mensaje):
    """Botón que comparte el PDF por WhatsApp con enlace de descarga y apertura directa."""
    with open(pdf_path, "rb") as f:
        b64 = base64.b64encode(f.read()).decode("utf-8")

    # Limpiar el texto del mensaje para URL
    texto_wa = texto_mensaje.replace(" ", "%20").replace("\n", "%0A")
    nombre_clean = nombre_archivo.replace("'", "\\'")
    ...
    <div style="font-family: 'Source Sans Pro', sans-serif; text-align: center;">
        <div style="display: flex; flex-direction: column; gap: 10px; max-width: 100%;">
            <!-- Botón principal: Descargar + Abrir WhatsApp -->
            <button onclick="descargarYWhatsApp()" 
            ...
            <!-- Botón secundario: Solo abrir WhatsApp -->
            <a href="https://api.whatsapp.com/send?text={texto_wa}" 
               target="_blank" 
               style="background-color: #128C7E; color: white; 
                      ...
                      text-align: center;">
                💬 Abrir WhatsApp (sin archivo adjunto)
            </a>
            
            <!-- Botón de descarga directa -->
            <a href="data:application/pdf;base64,{b64}" 
               download="{nombre_clean}" 
               style="background-color: #F5A623; color: white; 
                      border: none; padding: 12px 20px; border-radius: 8px; 
                      font-size: 15px; font-weight: 500; cursor: pointer; 
                      width: 100%; text-decoration: none; display: inline-block;
                      text-align: center;">
        ...
        <div id="estado-whatsapp" style="margin-top: 12px; font-size: 13px; color: #ccc; text-align: center; padding: 8px; background: #333; border-radius: 6px; display: none;">
            ✅ Archivo descargado. Abriendo WhatsApp...
        </div>
    </div>

    <script>
        const pdfData_{id_sufijo} = "{b64}";
        const nombreArchivo_{id_sufijo} = "{nombre_clean}";
        
        function descargarYWhatsApp_{id_sufijo}() {{
            try {{
                // Mostrar mensaje de estado
                const estado = document.getElementById('estado-whatsapp_{id_sufijo}');
                estado.style.display = 'block';
                estado.textContent = '⏳ Preparando descarga...';
                
                // Decodificar base64 y crear el blob del PDF
                const byteCharacters = atob(pdfData_{id_sufijo});
                ...
                link.download = nombreArchivo_{id_sufijo};
                document.body.appendChild(link);
                link.click();
                document.body.removeChild(link);
                
                estado.textContent = '✅ PDF descargado. Abriendo WhatsApp...';
                estado.style.color = '#F5A623';
                
                // Abrir WhatsApp después de la descarga
                setTimeout(() => {{
                    const waUrl = `https://api.whatsapp.com/send?text=${{encodeURIComponent('{texto_mensaje}')}}`;
                    window.open(waUrl, '_blank');
                    estado.textContent = '📲 WhatsApp abierto. Adjunta el archivo descargado manualmente.';
                    estado.style.color = '#25D366';
                }}, 1200);
...
def boton_compartir_whatsapp_excel(xlsx_path, nombre_archivo, texto_mensaje):
    """Botón que comparte el Excel por WhatsApp con enlace de descarga y apertura directa."""
    ...
    <script>
        const pdfDataExcel = "{b64}";
        const nombreArchivoExcel = "{nombre_clean}";
        
        function descargarYWhatsApp() {{
            try {{
                const estado = document.getElementById('estado-whatsapp');
                estado.style.display = 'block';
                estado.textContent = '⏳ Preparando descarga...';
                
                const byteCharacters = atob(pdfDataExcel);
                const byteNumbers = new Array(byteCharacters.length);
                for (let i = 0; i < byteCharacters.length; i++) {{
                    byteNumbers[i] = byteCharacters.charCodeAt(i);
                }}
                ...
                estado.textContent = '✅ Archivo descargado. Abriendo WhatsApp...';
                estado.style.color = '#F5A623';
...
def boton_compartir_whatsapp_fotos(imagenes, texto_mensaje):
    ...
    <div style="font-family: 'Source Sans Pro', sans-serif;">
      <button id="btn-compartir-fotos" style="
        ...
        🖼️ Enviar {n_fotos} foto(s) por WhatsApp
      </button>
      ...
      btnFotos.addEventListener("click", async () => {{
        try {{
          ...
          if (navigator.canShare && navigator.canShare({{ files }})) {{
            ...
            await navigator.share({{ title: "Fotos del informe de detectores de Rn", text: textoMensajeFotos }});
...
def pantalla_inicio():
    ...
    if st.button("➕ Nuevo centro", use_container_width=True):
...
def pantalla_centro():
    ...
    with st.expander("Datos del centro", expanded=True):
        ...
        with c2:
            widget_imagen("Imagen exterior", img_key, key_prefix=ns_centro + "_imgw", con_boton_quitar=False)
    ...
    if not detectores:
        ...
        if ultimo_pdf and os.path.exists(ultimo_pdf) and st.session_state.get("ultimo_pdf_centro") == cid:
            ...
            # Checklist de fotos del informe (imagen exterior del centro y
            # plano/situación/detector de cada detector) para elegir cuáles
            # se adjuntan junto al PDF al enviar por WhatsApp.
            fotos_disponibles = []
            ...
            for d in detectores:
                ...
                plano_d, foto_sit_d, foto_det_d = d[6], d[9], d[10]
                base_id = codigo_d or f"Detector {did_d}"
                if plano_d and os.path.exists(plano_d):
                    ext = os.path.splitext(plano_d)[1] or ".jpg"
                    fotos_disponibles.append({
                        "ruta": plano_d, "nombre_archivo": f"plano_det{did_d}{ext}",
                        "etiqueta": f"Plano — {base_id}",
                    })
            ...
            if fotos_disponibles:
                st.markdown(
                    '<p style="color:#F5A623; font-size:1.05rem; font-weight:700; margin:0 0 0.3rem 0;">'
                    '📷 Fotos del informe</p>',
                    unsafe_allow_html=True,
                )
        ...
        st.markdown("---")
        st.subheader("📊 Hoja de cálculo")
        if st.button("Generar Excel", type="primary"):
            try:
                nombre_xlsx = f"detectores_{_slug(nombre)}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.xlsx"
                ruta_xlsx = os.path.join(get_data_dir(), nombre_xlsx)
                with st.spinner("Generando hoja de cálculo (incrustando fotos)..."):
        ...
        if ultimo_excel and os.path.exists(ultimo_excel) and st.session_state.get("ultimo_excel_centro") == cid:
...
def pantalla_detector():
    ...
    if not st.session_state.get(init_key):
        if detector_id:
            ...
            (did, _, planta, sala, fecha, codigo, plano, px, py, foto_sit, foto_det, _,
             codigo_sala, profesionales_sala, hora_colocacion, turno_trabajo, nivel) = d
        else:
