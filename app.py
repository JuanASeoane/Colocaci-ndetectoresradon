...
st.markdown(
    ...
    /* Botones en azul (Nuevo centro, Guardar centro, Nuevo detector,
       Generar documentos): se marcan con type="tertiary" en Python,
       así el CSS engancha directamente al atributo real del botón en
       vez de depender de su posición en la página (más fiable). */
...
def boton_compartir_whatsapp_fotos(imagenes, texto_mensaje):
    ...
    checklist (sin el PDF), UNA A LA VEZ.

    IMPORTANTE: los navegadores solo permiten abrir el diálogo de
    compartir como reacción DIRECTA a un toque del usuario. En cuanto
    compartes una foto (se abre WhatsApp) y vuelves a la app, ya no se
    puede seguir compartiendo las demás automáticamente — el propio
    navegador lo bloquea por seguridad. Por eso este botón pide un
    toque por cada foto: en cuanto envías una, el botón pasa a mostrar
    la siguiente, y así hasta terminar todas.
    ...
    <script>
      ...
      function actualizarUI() {{
        ...
        progresoFotos.textContent = total > 1
          ? `Foto ${{indiceFoto + 1}} de ${{total}}: ${{archivosDataFotos[indiceFoto].name}}`
          : `Foto: ${{archivosDataFotos[indiceFoto].name}}`;
        btnFotos.textContent = total > 1
          ? `📲 Enviar esta foto (${{indiceFoto + 1}}/${{total}})`
          : "📲 Enviar foto por WhatsApp";
      }}
      ...
      btnFotos.addEventListener("click", async () => {{
        ...
        try {{
          const file = new File([b64ToBlobFotos(a.b64, a.type)], a.name, {{ type: a.type }});
          if (navigator.canShare && navigator.canShare({{ files: [file] }})) {{
            await navigator.share({{
              files: [file],
...
def pantalla_centro():
    ...
    with st.expander("Datos del centro", expanded=True):
        ...
        if st.button("💾 Guardar centro", type="tertiary"):
