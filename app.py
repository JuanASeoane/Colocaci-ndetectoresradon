...
st.markdown(
    ...
    /* Botón "Importar centro": fondo oscuro con las letras en amarillo
       (en vez del gris/blanco normal de los botones secundarios). */
    ...
    with st.container(border=True):
        ...
        with bc1:
            if st.button("➕ Nuevo centro", use_container_width=True, type="secondary"):
                ...
                st.rerun()
        with bc2:
            st.markdown('<div class="marcador-btn-importar"></div>', unsafe_allow_html=True)
...
def pantalla_centro():
    ...
    st.markdown(
        f'<p style="color:#F5A623; font-size:1.55rem; font-weight:800; '
        f'line-height:1.3; margin:0.5rem 0;">🏢 {html.escape(nombre or "")}</p>',
        unsafe_allow_html=True,
    )
        ...
        with st.container(border=True):
            if not detectores:
                ...
                for i, d in enumerate(detectores):
                    ...
                    etiquetas_r[i] = " · ".join(partes_i) + f"  (ID {did_i})"
