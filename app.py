    ...
    try:
        ...
        for idx, d in enumerate(detectores, 1):
            (did, _, planta, sala, fecha_det, codigo, _plano_antiguo, px, py, foto_sit, foto_det, _,
             codigo_sala, profesionales_sala, hora_colocacion, turno_trabajo, nivel, plano_centro_id) = d
            plano = None
            ...
            if hora_colocacion:
                fecha_y_hora_colocacion += f"<br/>{hora_colocacion}"
            ...
            if plano and os.path.exists(plano):
                titulo_ubicacion = f"Ubicacion en el plano ({nombre_plano_actual}):" if nombre_plano_actual else "Ubicacion en el plano:"
...
def generar_excel(centro_id, output_path):
    ...
    for col in range(14, len(headers) + 1):
    ...
    for d in detectores:
        (did, _, planta, sala, fecha_det, codigo, _plano_antiguo, punto_x, punto_y,
         ...
         hora_colocacion, turno_trabajo, nivel, plano_centro_id) = d
        plano_p = None
        if plano_centro_id:
            plano_info = get_plano_centro(plano_centro_id)
            if plano_info:
        ...
        with st.container(border=True):
            ...
            with pcol1:
                ...
                if ruta_plano_c and os.path.exists(ruta_plano_c):
                    st.image(ruta_plano_c, use_container_width=True)
                else:
